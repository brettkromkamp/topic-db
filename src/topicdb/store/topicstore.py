...
class TopicStore:
    ...
    def get_topic_base_names(self, map_identifier: int, identifier: str) -> list[BaseName]:
    ...
    def get_association(
        ...
        """Retrieve an association with optional resolution of related entities.

        Fetches an association and its related data (base names, members) from the database.
        Optionally resolves attributes and occurrences based on retrieval mode.

        Args:
            map_identifier (int): The identifier of the topic map
            identifier (str): The identifier of the association to retrieve
            scope (str, optional): Filter by scope. Defaults to None.
            language (Language, optional): Filter by language. Defaults to None.
            resolve_attributes (RetrievalMode): Whether to resolve attributes.
                Defaults to DONT_RESOLVE_ATTRIBUTES.
            resolve_occurrences (RetrievalMode): Whether to resolve occurrences.
                Defaults to DONT_RESOLVE_OCCURRENCES.

        Returns:
            Association | None: The retrieved association if found, None otherwise

        Raises:
            TopicDbError: If there is a database error or if the association member is missing

        Example:
            >>> store.get_association(1, "assoc-1", resolve_attributes=RetrievalMode.RESOLVE_ATTRIBUTES)
        """
        result = None
    ...
    def delete_association(self, map_identifier: int, identifier: str) -> None:
        """Delete an association and all its related data.

        This method removes:
        - The association topic
        - Associated base names
        - Association members
        - Related occurrences
        - Related attributes
        ...
        Args:
            map_identifier (int): The identifier of the topic map
            identifier (str): The identifier of the association to delete

        Raises:
            TopicDbError: If there is an error during database operations

        Note:
            This is a cascading delete operation that will remove all related data
        """
        connection = sqlite3.connect(self.database_path)
        try:
    ...
    def get_attribute(self, map_identifier: int, identifier: str) -> Attribute | None:
        ...
        Args:
            ...
            scope (str, optional): The scope of the attribute. Defaults to None.
            language (Language, optional): The language of the attribute. Defaults to None.

        Returns:
            Attribute: The requested attribute if found

        Raises:
            TopicDbError: If there is an error accessing the database
        """
        result = None
    ...
    def get_attributes(
        ...
        """Get all attributes matching the specified criteria.

        Args:
            map_identifier (int): The identifier of the topic map
            entity_identifier (str): The identifier of the entity
            scope (str, optional): Filter by attribute scope. Defaults to None.
            language (Language, optional): Filter by attribute language. Defaults to None.

        Returns:
            list[Attribute]: List of matching attributes
